...
const pdfProgress = ref({ current: 0, total: 0 })
const pdfPageCounts = ref<Map<File, number>>(new Map())
  ...
  if (pdfFiles.length > 0) {
    ...
    try {
      for (const pdf of pdfFiles) {
        ...
        try {
          const result = await splitPdf(pdf, (current, total) => {
            pdfProgress.value = { current, total }
          })
          // Track page count for the first page file (used for badge display)
          if (result.files.length > 0) {
            pdfPageCounts.value.set(result.files[0], result.pageCount)
          }
          selectedFiles.value.push(...result.files)
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err)
          error.value = `Failed to process "${pdf.name}": ${msg}`
        }
      }
    } finally {
...
function removeFile(index: number) {
  ...
  if (removed) {
    ...
    pdfPageCounts.value.delete(removed)
  }
}
function clearAll() {
  selectedFiles.value.forEach(revokePreview)
  selectedFiles.value = []
  pdfPageCounts.value.clear()
}
...
<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-muted-foreground">Upload scanned images or PDFs for OCR processing and indexing.</p>
    </div>
    ...
    <template v-if="!result">
      ...
      >
        <CardContent class="flex flex-col items-center justify-center py-12">
          ...
          <p class="mb-1 text-sm font-medium">Drag and drop images or PDFs here</p>
      ...
      <Card v-if="pdfProcessing" class="border-primary/30">
        <CardContent class="flex items-center gap-3 py-4">
          <Loader2 class="h-5 w-5 animate-spin text-primary" aria-hidden="true" />
          <div class="flex-1">
            <p class="text-sm font-medium">Splitting PDF...</p>
            <p class="text-xs text-muted-foreground">
              {{ pdfProgress.current }}/{{ pdfProgress.total }} pages rendered
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card v-if="selectedFiles.length > 0">
        <CardHeader class="pb-3">
          <div class="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" :disabled="uploading || pdfProcessing" @click="clearAll">Clear All</Button>
          </div>
        </CardHeader>
        <CardContent>
          <div class="grid max-h-96 grid-cols-3 gap-2 overflow-auto sm:grid-cols-4 md:grid-cols-5">
            ...
            >
              ...
              />
              <span
                v-if="pdfPageCounts.has(file)"
                class="absolute left-1 top-1 rounded-full bg-primary px-1.5 py-0.5 text-[9px] font-medium text-primary-foreground"
              >
                {{ pdfPageCounts.get(file) }} pages
              </span>
              ...
              <button
                ...
                :disabled="uploading || pdfProcessing"
                @click="removeFile(i)"
              >
          ...
          <Button
            v-else
            class="mt-4 w-full"
            :disabled="selectedFiles.length === 0 || pdfProcessing"
            @click="handleUpload"
          >
